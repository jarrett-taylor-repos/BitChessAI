...
class Board {
    ...
    public:
        ...
        vector<pair<int,int>> setBishopAttacker(Square*);
        vector<pair<int,int>> setRookAttacker(Square*);
        vector<pair<int,int>> setKnightAttacker(Square*);
        vector<pair<int,int>> setPawnAttacker(Square*);
        vector<pair<int,int>> setKingAttacker(Square*);
        vector<pair<int,int>> setSquareAttack(Square*);

        void setAttackers(Square*);
        void setAllAttackers();
        void removeAllAttackers();
        vector<Square*> attackers(Square*s);

        Square* getKing(Color);
        Piece getPawnPromotion(Notation);

        pair<bool, Square*> isBetweenKingandAttacker(Square*, Square*, Square*);
        vector<Square*> isInCheck();
...
Board::Board(string fen) {
    ...
    allPGN = "";
    lastPGNmove = "";
    turnNum = moveColor == WHITE ? fullTurnNum*2-1: fullTurnNum*2;
...
vector<pair<Square*, Square*>> Board::getAllMoves() {
    ...
    setAllAttackers();
    vector<Square*> checks = isInCheck();
...
vector<pair<Square*, Square*>> Board::KingMoves(Square*sq){
    ...
    for(int i = 0; i < pairs.size(); i++) { 
        ...
        if(temp.first != -1) {
            bool notFutureCheck = true;
            Square* end = getSquare(temp.first, temp.second);
            int endx = end->getx();
            int endy = end->gety();
            vector<Square*> testcolor = end->getAllAttackers();
            for(int j = 0; j < testcolor.size(); j++) {
                if(testcolor[j]->getColor() != moveColor) {
                    notFutureCheck = false;
                }
            }
            if(notFutureCheck) {
...
pair<bool, vector<pair<Square*, Square*>>> Board::isSquarePinned(Square* s) {
    Piece p = s->getPiece();
    Color c = s->getColor();
    vector<Square*> attackers = s->getAttackers();
...
vector<Square*> Board::isInCheck() {
    vector<Square*> checks = getKing(moveColor)->getAttackers();
    return checks;
}
...
vector<bool> Board::canWhiteCastle() {
    ...
    bool no_checks_short = 
        attackers(getSquare(e1)).size() == 0 && 
        attackers(getSquare(f1)).size() == 0 && 
        attackers(getSquare(g1)).size() == 0;
    bool no_checks_long = 
        attackers(getSquare(e1)).size() == 0 &&  
        attackers(getSquare(d1)).size() == 0 &&  
        attackers(getSquare(c1)).size() == 0;
...
vector<bool> Board::canBlackCastle() {
    ...
    bool no_checks_short = 
        attackers(getSquare(e8)).size() == 0 && 
        attackers(getSquare(f8)).size() == 0 &&  
        attackers(getSquare(g8)).size() == 0;
    bool no_checks_long = 
        attackers(getSquare(e8)).size() == 0 &&  
        attackers(getSquare(d8)).size() == 0 &&  
        attackers(getSquare(c8)).size() == 0;
...
vector<pair<int,int>> Board::setBishopAttacker(Square*s) {
    int x = s->getx();
    int y = s->gety();
    vector<pair<int, int>> pairs;
    ...
    for(int i = 1; i < 8; i++) {
        ...
        if(isInRange(tempx, tempy)) {
            Square* temp = getSquare(tempx, tempy);
            if(temp->getColor() == s->getColor()) {
                pair<int, int> newpair = make_pair(tempx, tempy);
                pairs.push_back(newpair);
                break;
            } else if(temp->getColor() != NONE && temp->getColor() != s->getColor()) {
                if(temp->getPiece() == KING && temp->getColor() != s->getColor()) {
                    pair<int, int> newpair = make_pair(tempx, tempy);
                    pairs.push_back(newpair);
                } else {
                    pair<int, int> newpair = make_pair(tempx, tempy);
...
vector<pair<int,int>> Board::setRookAttacker(Square*square) {
    int x = square->getx();
    int y = square->gety();
    vector<pair<int, int>> pairs;
    ...
    for(int i = x+1; i < 8; i++) {
        ...
        int tempy = y;
        Square* temp = getSquare(tempx, tempy);
        if(temp->getColor() == square->getColor()) {
            pair<int, int> newpair = make_pair(tempx, tempy);
            pairs.push_back(newpair);
            break;
        } else if(temp->getColor() != NONE && temp->getColor() != square->getColor()) {
            if(temp->getPiece() == KING && temp->getColor() != square->getColor()) {
                pair<int, int> newpair = make_pair(tempx, tempy);
                pairs.push_back(newpair);
            } else {
                pair<int, int> newpair = make_pair(tempx, tempy);
...
vector<pair<int,int>> Board::setKnightAttacker(Square*square) {
    ...
    return searched_pairs;
}
vector<pair<int,int>> Board::setPawnAttacker(Square*square) {
    int currx = square->getx();
    int curry = square->gety();
...
vector<pair<int,int>> Board::setKingAttacker(Square*square) {
    int currx = square->getx();
    int curry = square->gety();
    ...
    for(int i = 0; i < pairs.size(); i++) {
        ...
        bool onboard = isInRange(tempx, tempy);
        if(onboard) {
            searched_pairs.push_back(make_pair(tempx, tempy));
        }
    }
...
void Board::setAttackers(Square* att) { 
    Piece p = att->getPiece();
    vector<pair<int,int>> setAtts;
    vector<pair<int,int>> bishops;
    vector<pair<int,int>> rooks;
    switch(p){
        case KING:
            setAtts = setKingAttacker(att);
            break;
        case QUEEN:
            bishops = setBishopAttacker(att);
            rooks = setRookAttacker(att);
            setAtts.insert(setAtts.end(), bishops.begin(), bishops.end());
            setAtts.insert(setAtts.end(), rooks.begin(), rooks.end());
            break;
        case ROOK:
            setAtts = setRookAttacker(att);
            break;
        case BISHOP:
            setAtts = setBishopAttacker(att);
            break;
        case KNIGHT:
            setAtts = setKnightAttacker(att);
            break;
        case PAWN:
            setAtts = setPawnAttacker(att);
            break;
        default:
            break;
    }

    for(int i = 0; i < setAtts.size(); i++) {
        pair<int,int> temp = setAtts[i];
        int tempx = temp.first;
        int tempy = temp.second;
        Square* setatt = getSquare(tempx, tempy);
        setatt->addAttacker(att);
    }
    return;
}

void Board::setAllAttackers() {
    removeAllAttackers();
    for(int i = 0; i < allPieces.size(); i++) {
        Square * temp = allPieces[i];
        setAttackers(temp);
...
void Board::removeAllAttackers() {
    for(int i = 0; i < 8; i++) {
        for(int j = 0; j < 8; j++) {
            Square* temp = getSquare(i, j);
            temp->removeAttackers();
        }
    }

}

vector<Square*> Board::attackers(Square*s) {
    vector<Square*> test = s->getAttackers();
    if(s->getColor() != NONE) {
        return test;
    }
    vector<Square*> oppColor;
    for(int i = 0; i < test.size(); i++) {
        if(test[i]->getColor() != moveColor) {
            oppColor.push_back(test[i]);
        }
    }
    return oppColor;
} 
