...
class Square {
  ...
  public:
    ...
    vector<Square*> getAttackers();
    vector<Square*> getAllAttackers();
    void addAttacker(Square *);
...
//attackers 
vector<Square*> Square::getAttackers() {
  vector<Square*> oppositeColor;
  for(int i = 0; i < attackers.size(); i++) { 
    if(attackers[i]->getColor() != color){
      oppositeColor.push_back(attackers[i]);
    }
  }
  return oppositeColor;
}
vector<Square*> Square::getAllAttackers() {
  return attackers;
}

void Square::addAttacker(Square *s) {
  attackers.push_back(s);
}
void Square::removeAttackers() {
  attackers.clear();
  attackers.resize(0);
}
