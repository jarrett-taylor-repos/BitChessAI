...
int main() {
    ...
    // string testPgnAmbuiguity = "k6B/8/1p6/p1p4p/P1P1P2P/1P1P2P1/7R/3K1NR1 w - - 7 72";
    // Board amb(testPgnAmbuiguity);
    // amb.makeMoveAndPrint(h2, g2);
    // cout << amb.getLastPGNmove() << endl;

    //test from correct chess pos
    // string test1 = "rnbqkbnr/1ppppppp/p7/8/8/3P4/PPP1PPPP/RNBQKBNR w KQkq - 0 2";//got 26 need 27
    // testBoardPos(test1); //problem is king moves

    // string test2 = "rnbqkbnr/ppp1pppp/3p4/1B6/8/4P3/PPPP1PPP/RNBQK1NR b KQkq - 1 2";//get 19, need 17
    // testBoardPos(test2);
    
    string test3 = "rnQq1k1r/pp2bppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R b KQ - 0 8";
    Board t(test3);
    t.print();
    vector<pair<Notation, Notation>> notmoves = t.returnNotationMoves();
    cout << notmoves.size() << endl;
    for(int i = 0; i < notmoves.size(); i++) {
        cout << notmoves[i].first << notmoves[i].second << endl;
    }
    Board t2 = t;
    t2.print();
    notmoves = t2.returnNotationMoves();
    cout << notmoves.size() << endl;
    for(int i = 0; i < notmoves.size(); i++) {
        cout << notmoves[i].first << notmoves[i].second << endl;
    }
    ...
    t3 = t2;
    t3.print();
    notmoves = t3.returnNotationMoves();
    cout << notmoves.size() << endl;
    for(int i = 0; i < notmoves.size(); i++) {
        cout << notmoves[i].first << notmoves[i].second << endl;
    }
